...
async def ticker(states, notifier, focus):
    ...
    while True:
        ...
        if GHOST_CURSOR and now - last_ghost >= 1.0 / GHOST_HZ:
            last_ghost = now
            dragging = next(
                (s for s in states.values() if s.active or s.toggled),
                None,
            )
    ...
    try:
        async for ev in dev.async_read_loop():
            ...
            if ev.type == e.EV_REL and ev.code in (e.REL_X, e.REL_Y):
                if st.toggled or st.pending or st.active:
                    # Keep the real pointer at the origin so wheel events
                    # stay on the window you started in. The overlay draws
                    # a ghost/circle from the pos stream.
                    _accumulate(st, ev)
                    continue
            if ev.type == e.EV_SYN:
